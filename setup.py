...
if path.exists(here + "/.git"):
    init_file = None
    with open(path.join(here, 'fortiori/__init__.py'), 'r', encoding='utf-8') as f:
        init_file = f.read()
    if "@@VERSION@@" in init_file:
        with open(path.join(here, 'fortiori/__init__.py'), 'w', encoding='utf-8') as f:
...
setup(
    ...
    url='https://github.com/keotl/fortiori',
